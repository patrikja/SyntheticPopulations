...
n_combinations = np.product(map(lambda e: len(e), sample_bin_names))
...
# A list of indices for sample_bin_names. This list is updated for every
#   combination of bins below, and used so that each combination is expressed
#   by a unique combination of bin indices.
# TODO: Is is important to have a sparse matrix representation here? It adds
...
for combination_no in range(0, n_combinations):
  ...
  for attribute_no in range(0, n_attributes):
    # The current attribute for the current combination is given its value
    combinations[combination_no][attribute_no] = bin_indices[attribute_no]

    # The df is filtered to remove all rows where the attribute differs from
    #   the value given on the row above.
    # TODO: Simplify code + explain.
    micro_df_sub =   micro_df_sub.loc[  micro_df_sub[sample_attributes[attribute_no]] ==
                                        sample_bin_names[attribute_no][bin_indices[attribute_no]]
                                     ]

  # The rows still remaining in the df are the ones where all the attribute
  #   values match the signature given in bin_indices.
  counts[combination_no] = len(micro_df_sub.index)
