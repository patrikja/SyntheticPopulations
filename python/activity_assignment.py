...
class Person:
  def __init__(self, person_id, household_id, attributes, activities):
    ...
    #Array with ones ond zeros where all bin values, exept the first, for all attributes are represented.
    #One means that that bin value is the matching one for this person. If there are only zeros for an
    #attribute it means that the first bin value is used.
...
.format(self.person_id, self.household_id, self.attributes, self.activities)
  ...
  #Fitted activity time is y = x*b, where y is a vector of times for different categories, x is a vector of
  #ones and zeros, representing the presence of attributes (a 1 is added for the interception) and b is a
  #matrix with the the linear coefficients.
  def assign_fitted_time(self, beta):
    self.fitted_activity_times = np.matmul(beta, np.hstack((1, self.bins)))

  #Calculate the distance between two persons as the (Euclidian) distance between their fitted
  #activity time vectors. 
  #TODO: Replace with Mahalanobis distance instead of Euclidian
  def distance(self, other_person):
    return np.linalg.norm(self.fitted_activity_times-other_person.fitted_activity_times)
...
class Household:
  ...
  #For every person in one of the households, take the smallest distance between it and any person in the other household.
  #The household distance is the biggest of the person-person distances for all persons in the household.
...
for index, attribute_row in survey_attributes_df.iterrows():
  ...
  while survey_activities_df['person_id'].iloc[activity_row_no] == attribute_row['person_id']:
    activities.append(Activity(survey_activities_df['activity_type'].iloc[activity_row_no], survey_activities_df['start_time'].iloc[activity_row_no], survey_activities_df['duration'].iloc[activity_row_no], survey_activities_df['location'].iloc[activity_row_no]))
    activity_row_no += 1

  attributes = map(lambda a: attribute_row[a], attribute_names)
  survey_persons.append(Person(attribute_row['person_id'], attribute_row['household_id'], attributes, activities))
...
#Create a dataframe with activity times and attributes. The attributes are represented as dummy variables
#by the vector of zeros and ones created above.
...
act_df=act_df.fillna(0) #TODO: This is needed to make the output of the fitting nice. WHY???
...
#Then, for each synthetic person in the synthetic household, find the survey person
#in the selected survey household that it is closest to and copy the survey person's 
#schedule to the synthetic person
...
synthetic_activities_df = pd.DataFrame(columns=['person_id', 'household_id', 'activity_type', 'start_time', 'duration', 'location'])
for person in synthetic_persons:
  for activity in person.activities:
    row = pd.DataFrame(columns=['person_id', 'household_id', 'activity_type', 'start_time', 'duration', 'location'])
